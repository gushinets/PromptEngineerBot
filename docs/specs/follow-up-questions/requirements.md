...
**User Story:** As a user, I want to start follow-up questions immediately after accepting them, so that I can quickly refine my prompt without additional steps.

#### Acceptance Criteria

1. WHEN the user clicks the [ДА] button THEN the system SHALL start a new conversation for follow-up questions immediately
2. WHEN starting follow-up conversation THEN the system SHALL load the follow-up questions prompt using PromptLoader.followup_prompt as system context
3. WHEN system context is loaded THEN the system SHALL add the cached improved prompt as user context
