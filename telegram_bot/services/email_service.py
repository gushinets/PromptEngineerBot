...
class EmailService:
    ...
    async def send_report_email(
        ...
        report_date: str,
    ) -> dict[str, EmailDeliveryResult]:
        """
        Send marketing report email with CSV attachments to multiple recipients.

        Args:
            recipient_emails: List of recipient email addresses
            attachments: List of (filename, content) tuples for CSV attachments
            report_date: Report date string for email subject (YYYY-MM-DD format)

        Returns:
            Dictionary mapping email addresses to their delivery results
        """
        results: dict[str, EmailDeliveryResult] = {}

        if not recipient_emails:
            logger.warning("REPORT_EMAIL_SKIP: No recipient emails configured")
            return results

        if not attachments:
            logger.warning("REPORT_EMAIL_SKIP: No attachments provided")
            return results

        logger.info(
            f"REPORT_EMAIL_START: Sending report email to {len(recipient_emails)} recipients "
            f"with {len(attachments)} attachments for date {report_date}"
        )

        # Generate subject
        subject = f"Marketing Reports - {report_date}"

        # Generate email body
        attachment_list = "\n".join([f"  - {filename}" for filename, _ in attachments])
        html_body = f"""
        <html>
        <body>
            <h2>Daily Marketing Reports</h2>
            <p>Please find attached the marketing reports for <strong>{report_date}</strong>.</p>
            <p>Attached files:</p>
            <ul>
                {"".join([f"<li>{filename}</li>" for filename, _ in attachments])}
            </ul>
            <p>This is an automated report generated by the Marketing Reports System.</p>
        </body>
        </html>
        """
        plain_body = f"""Daily Marketing Reports

Please find attached the marketing reports for {report_date}.

Attached files:
{attachment_list}

This is an automated report generated by the Marketing Reports System.
"""
        ...
        for to_email in recipient_emails:
            try:
                logger.info(
                    f"REPORT_EMAIL_COMPOSE: Preparing report email for {self.mask_email(to_email)}"
                )

                # Generate idempotency hash
                content_preview = f"REPORT:{report_date}:{len(attachments)}"
                email_hash = self._generate_email_hash(to_email, subject, content_preview)

                # Check idempotency
                if await self._is_email_already_sent(email_hash):
                    logger.info(
                        f"REPORT_EMAIL_DUPLICATE_BLOCKED: Duplicate report email blocked for {self.mask_email(to_email)}"
                    )
                    results[to_email] = EmailDeliveryResult(
                        success=True, message_id="duplicate_blocked", delivery_time_ms=0
                    )
                    continue

                # Send email with attachments
                result = await self._send_report_email_with_attachments(
                    to_email=to_email,
                    subject=subject,
                    html_body=html_body,
                    plain_body=plain_body,
                ...
                results[to_email] = result

                # Log delivery status
                if result.success:
                    logger.info(
                        f"REPORT_EMAIL_SENT: Report email delivered to {self.mask_email(to_email)} "
                        f"in {result.delivery_time_ms}ms"
                    )
                else:
                    logger.error(
                        f"REPORT_EMAIL_FAILED: Failed to send report email to {self.mask_email(to_email)} - {result.error}"
                    )

            except Exception as e:
                logger.error(
                    f"REPORT_EMAIL_ERROR: Unexpected error sending report email to {self.mask_email(to_email)} - {e!s}"
                )
                results[to_email] = EmailDeliveryResult(
        ...
        logger.info(
            f"REPORT_EMAIL_COMPLETE: Sent to {successful}/{len(results)} recipients "
            f"({failed} failed)"
        )
    ...
    async def _send_report_email_core(
        self,
        to_email: str,
        subject: str,
        html_body: str,
        ...
        """Core report email sending logic with attachments and tenacity retry."""
        start_time = time.time()
        retry_state = getattr(self._send_report_email_core.retry, "statistics", {})
        attempt_number = retry_state.get("attempt_number", 1) if retry_state else 1

        async with self._get_smtp_connection() as smtp:
            # Create MIME message with mixed type for attachments
    ...
    async def _send_report_email_with_attachments(
        self,
        to_email: str,
        subject: str,
        html_body: str,
        ...
        """Send report email with attachments using retry logic."""
        try:
            message_id, delivery_time_ms, retry_count = await self._send_report_email_core(
                to_email=to_email,
                subject=subject,
                html_body=html_body,
                plain_body=plain_body,
            ...
            if retry_count > 0:
                logger.info(
                    f"REPORT_EMAIL_SUCCESS: Report email delivered to {self.mask_email(to_email)} "
                    f"in {delivery_time_ms}ms after {retry_count} retries"
                )
            else:
                logger.info(
                    f"REPORT_EMAIL_SUCCESS: Report email delivered to {self.mask_email(to_email)} "
                    f"in {delivery_time_ms}ms"
                )
            ...
            error_msg = str(e)
            logger.error(
                f"REPORT_EMAIL_FAILED: All retries exhausted for {self.mask_email(to_email)} "
                f"after {retry_count} attempts - {error_msg}"
            )
