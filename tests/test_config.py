...
class TestBotConfig:
    ...
    def test_from_env_missing_openai_key(self):
        ...
        with patch.dict(os.environ, {
            ...
            with pytest.raises(ValueError, match="OPENAI_API_KEY is required when using OpenAI backend"):
                BotConfig.from_env()
    ...
    def test_from_env_missing_openrouter_key(self):
        ...
        with patch.dict(os.environ, {
            ...
            with pytest.raises(ValueError, match="OPENROUTER_API_KEY is required when using OpenRouter backend"):
                BotConfig.from_env()
    ...
    def test_validate_gsheets_missing_spreadsheet(self):
        ...
        with pytest.raises(ValueError, match="Google Sheets logging enabled but no spreadsheet ID or name provided"):
            config.validate()
    ...
    def test_validate_gsheets_missing_credentials(self):
        ...
        with pytest.raises(ValueError, match="Google Sheets logging enabled but no credentials provided"):
            config.validate()
