...
from src.messages import BTN_CRAFT, BTN_LYRA, BTN_GGL, BTN_RESET
...
class TestBotHandler:
    ...
    def mock_llm_client(self):
        ...
        client.get_last_usage = MagicMock(return_value={'prompt_tokens': 10, 'completion_tokens': 20, 'total_tokens': 30})
        return client
    ...
    @pytest.mark.asyncio
    async def test_handle_message_method_selection_craft(self, bot_handler, mock_update, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_handle_message_method_selection_lyra(self, bot_handler, mock_update, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_handle_message_invalid_method_selection(self, bot_handler, mock_update, mock_context):
    ...
    async def test_process_with_llm_improved_prompt(self, bot_handler, mock_update, mock_context):
        ...
        bot_handler.llm_client.send_prompt.return_value = "<IMPROVED_PROMPT>Improved prompt here</IMPROVED_PROMPT>"
        ...
        assert bot_handler.conversation_manager.get_transcript(user_id) == []
        assert bot_handler.state_manager.get_user_state(user_id).waiting_for_prompt is True
    ...
    def test_log_conversation_totals(self, bot_handler):
        ...
        sheets_logger.assert_called_once_with("conversation_totals", {
            "BotID": "test_bot",
            "TelegramID": user_id,
            "LLM": "TEST:test-model",
