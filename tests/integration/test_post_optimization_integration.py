...
class TestPostOptimizationSystemIntegration:
    ...
    async def test_complete_followup_to_email_flow(self):
        ...
        with patch("telegram_prompt_bot.core.bot_handler.get_container") as mock_container:
            ...
            # Mock health monitor
            with patch("telegram_prompt_bot.infrastructure.health_checks.get_health_monitor") as mock_health:
    ...
    async def test_existing_email_flow_unaffected(self):
        ...
        with patch("telegram_prompt_bot.core.bot_handler.get_container") as mock_container:
            ...
            with patch("telegram_prompt_bot.infrastructure.health_checks.get_health_monitor") as mock_health:
                ...
                email_flow.start_email_flow.assert_called_once_with(
                    update, context, user_id
                )
    ...
    async def test_button_isolation_between_flows(self):
        ...
        from telegram_prompt_bot.utils.messages import FOLLOWUP_CHOICE_KEYBOARD, SELECT_METHOD_KEYBOARD
    ...
    async def test_state_management_isolation(self):
        ...
        with (
            patch("telegram_prompt_bot.email_flow.get_auth_service") as mock_auth,
            patch("telegram_prompt_bot.email_flow.get_email_service") as mock_email,
            ...
            await orchestrator.start_post_optimization_email_flow(
                update, context, user_id
            )
    ...
    async def test_concurrent_user_isolation(self):
        ...
        with (
            ...
            await orchestrator.start_post_optimization_email_flow(
                update1, context, 12345
            )
            await orchestrator.start_post_optimization_email_flow(
                update2, context, 67890
            )
...
class TestPostOptimizationEmailTemplateValidation:
    ...
    def test_single_result_template_completeness(self):
        ...
        method_name = "CRAFT"
        optimized_result = (
            "Создайте подробный план маркетинга для технологического стартапа..."
        )
...
class TestPostOptimizationRegressionPrevention:
    ...
    def test_existing_button_definitions_unchanged(self):
        ...
        from telegram_prompt_bot.utils.messages import BTN_CRAFT, BTN_EMAIL_DELIVERY, BTN_GGL, BTN_LYRA
    ...
    def test_existing_keyboard_layouts_unchanged(self):
        ...
        method_buttons = [
            button.text for row in SELECT_METHOD_KEYBOARD.keyboard for button in row
        ]
    ...
    def test_message_constants_backward_compatibility(self):
        ...
        assert (
            isinstance(OTP_VERIFICATION_SUCCESS, str)
            and len(OTP_VERIFICATION_SUCCESS) > 0
        )
        assert (
            isinstance(EMAIL_OPTIMIZATION_SUCCESS, str)
            and len(EMAIL_OPTIMIZATION_SUCCESS) > 0
        )
