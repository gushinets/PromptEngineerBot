...
class TestForceReplyFunctionality:
    """Test cases for ForceReply functionality."""

    def test_followup_prompt_input_message_exists(self):
        """Test that FOLLOWUP_PROMPT_INPUT_MESSAGE constant is defined and localized."""
        assert FOLLOWUP_PROMPT_INPUT_MESSAGE is not None
        assert len(FOLLOWUP_PROMPT_INPUT_MESSAGE) > 0

        # Should contain key phrases based on current language
        if LANGUAGE == "ru":
            assert "Поменяйте или добавьте" in FOLLOWUP_PROMPT_INPUT_MESSAGE
            assert "промпта" in FOLLOWUP_PROMPT_INPUT_MESSAGE
            assert "отправьте" in FOLLOWUP_PROMPT_INPUT_MESSAGE
        else:
            assert "Modify or add" in FOLLOWUP_PROMPT_INPUT_MESSAGE
            assert "prompt" in FOLLOWUP_PROMPT_INPUT_MESSAGE
            assert "send" in FOLLOWUP_PROMPT_INPUT_MESSAGE

    def test_create_prompt_input_reply_basic(self):
        """Test basic ForceReply creation with improved prompt."""
        improved_prompt = "Write a detailed story about adventure"

        force_reply = create_prompt_input_reply(improved_prompt)

        # Should be a ForceReply object
        from telegram import ForceReply

        assert isinstance(force_reply, ForceReply)

        # Should have the improved prompt as placeholder
        assert force_reply.input_field_placeholder == improved_prompt

        # Should not be selective
        assert force_reply.selective is False

    def test_create_prompt_input_reply_short_prompt(self):
        """Test ForceReply creation with short prompt."""
        improved_prompt = "Hello"
    ...
    def test_create_prompt_input_reply_long_prompt(self):
        """Test ForceReply creation with long prompt."""
        improved_prompt = (
            "This is a very long improved prompt that contains many details and instructions for the user to follow when creating their content. "
            * 5
        )
    ...
    def test_create_prompt_input_reply_empty_prompt(self):
        """Test ForceReply creation with empty prompt."""
        improved_prompt = ""

        force_reply = create_prompt_input_reply(improved_prompt)

        assert force_reply.input_field_placeholder == ""
        assert isinstance(force_reply, ForceReply)

    def test_create_prompt_input_reply_whitespace_prompt(self):
        """Test ForceReply creation with whitespace-only prompt."""
        improved_prompt = "   \n\t   "
    ...
    def test_create_prompt_input_reply_special_characters(self):
        """Test ForceReply creation with special characters in prompt."""
        improved_prompt = (
            'Prompt with "quotes", symbols: @#$%^&*(), and unicode: café 🚀'
        )
    ...
    def test_create_prompt_input_reply_multiline_prompt(self):
        """Test ForceReply creation with multiline prompt."""
        improved_prompt = """Line 1 of the prompt
Line 2 with more details
Line 3 with final instructions"""
    ...
    def test_create_prompt_input_reply_markdown_characters(self):
        """Test ForceReply creation with markdown characters in prompt."""
        improved_prompt = "Prompt with *emphasis*, **bold**, `code`, and _italic_ text"

        force_reply = create_prompt_input_reply(improved_prompt)

        # Should preserve markdown characters as-is (no escaping needed for ForceReply)
        assert force_reply.input_field_placeholder == improved_prompt
        assert isinstance(force_reply, ForceReply)

    def test_create_prompt_input_reply_very_long_prompt(self):
        """Test ForceReply creation with very long prompt (edge case)."""
        # Create a prompt that's longer than typical Telegram limits
        base_text = "This is a segment of a very long prompt. "
        improved_prompt = base_text * 100  # ~4000 characters
    ...
    def test_create_prompt_input_reply_selective_false(self):
        """Test that ForceReply is created with selective=False."""
        improved_prompt = "Test prompt"

        force_reply = create_prompt_input_reply(improved_prompt)

        # Should always be non-selective to work for all users
        assert force_reply.selective is False

    def test_create_prompt_input_reply_return_type(self):
        """Test that function returns correct type."""
        improved_prompt = "Test prompt"

        force_reply = create_prompt_input_reply(improved_prompt)

        # Should return ForceReply instance
        from telegram import ForceReply

        assert type(force_reply) is ForceReply
