...
from telegram_prompt_bot.database.models import init_database_from_config
from telegram_prompt_bot.utils.graceful_degradation import init_degradation_manager
from telegram_prompt_bot.infrastructure.gsheets_logging import build_google_sheets_handler_from_env
from telegram_prompt_bot.infrastructure.health_checks import init_health_monitor
from telegram_prompt_bot.llm.factory import LLMClientFactory
from telegram_prompt_bot.monitoring.logging_utils import setup_application_logging

# NOTE: Specific message constants are imported where needed inside handlers
from telegram_prompt_bot.infrastructure.redis_client import get_redis_client, init_redis_client
...
async def main():
    ...
    try:
        ...
        if config.email_enabled:
            ...
            from telegram_prompt_bot.auth.service import init_auth_service

            init_auth_service(config)
            logger.info("Auth service initialized successfully")

            # Initialize audit service
            from telegram_prompt_bot.monitoring.audit_service import init_audit_service
            ...
            from telegram_prompt_bot.email.service import init_email_service

            init_email_service(config)
            logger.info("Email service initialized successfully")

            # Initialize email flow orchestrator after all services are ready
            from telegram_prompt_bot.email.flow import init_email_flow_orchestrator
